...
// const {authenticateSocket, authenticateUser} = require('./middlewares/user.middleware.js')
...
// const io = new Server(server, {
//   cors: {
//     origin: "http://localhost:3000", // Allow frontend to access the socket server
//     methods: ["GET", "POST"],
//     credentials: true
//   }
// });

// let onlineUsers = {}; // Track online users by socket.id
// let socketIds = [];
// let usersMap = {}; // userId ↔ socketId mapping

// // Helper function to remove socket ID from array
// Array.prototype.remove = function(value) {
//   let index = this.indexOf(value);
//   if (index !== -1) {
//     this.splice(index, 1);
...
//   // 💥 Disconnect old socket if user already exists
//   const oldSocketId = usersMap[userId];
//   if (oldSocketId && oldSocketId !== newSocketId) {
//     const oldSocket = io.sockets.sockets.get(oldSocketId);
//     if (oldSocket) {
//       oldSocket.emit("force_disconnect"); // Notify old tab
//       oldSocket.disconnect(true); // Disconnect it
//     }
//   }

//   // ✅ Register new socket
//   usersMap[userId] = newSocketId;

//   if (!socketIds.includes(newSocketId)) {
//     socketIds.push(newSocketId);
//   }

//   // Send socket ID to client
//   socket.emit("new_user", newSocketId);

//   // Client asks for their socket ID again
//   socket.on("get_user_id", () => {
//     socket.emit("new_user", socket.id);
//   });
...
//   socket.on("sendPrivateMessage", ({ userId: targetUserId, inputValue }) => {
//     const targetSocketId = usersMap[targetUserId]; // make sure usersMap is mapping userId -> socketId
